...
Chen2022 SCADA xlsx data are available; the validation file
additionally lists three synthetic duplicates that lack
measured power data)
...
total~\cite{ruan2025,he2023}; combined wind and solar capacity
...
et al.~\cite{bilgili2024} achieved 96.07\% accuracy with
...
proliferation of systematic reviews~\cite{resch2023heliyon,jurasz2025}
...
of MCDM. Bilgili et al.~\cite{bilgili2024} achieved 96.07\%
...
deep learning~\cite{shwartz2022,borisov2022} together provide
...
surveys of tabular data~\cite{borisov2022,shwartz2022} and the
...
\midrule
1 & 9-row validation file (6 SCADA, 3 coord.\ duplicates) & Independent spatial hold-out &
...
3 & OSM 1,115 & Stratified provincial &
  Num. only & 0.8838$^\S$ & RandomForest baseline (§\ref{sec:baselines}) \\
4 & OSM 1,115 & Stratified provincial &
  Num.+policy & 0.8386$^\ddagger$ & Primary result, ablation, SHAP \\
\bottomrule
\multicolumn{6}{p{0.95\textwidth}}{\small
  $^\ddagger$ Experiment~4 uses the constrained negative
  sampling protocol of Appendix~\ref{sec:appendix}: a
  10\,km spatial buffer from positives, slope $\leq 25^\circ$,
  elevation $\leq 3{,}500$\,m, and settlement distance
  $\geq 0.5$\,km. The reduction from 0.8595 (unconstrained) to
  0.8386 reflects increased task difficulty, not model
  degradation; AP rises from 0.3023 to 0.7333 as the positive
  rate is corrected from 11\% to 33\%.
  $^\S$ Experiment~3 and the RandomForest row of
  Section~\ref{sec:baselines} report the same numerical-only
  model class (random forest, $n=500$) on the same
  constrained protocol; the AUC values match to 0.1\%.
  Earlier-draft Experiment~3 reported $0.8820$ under a
  different seed; that number is superseded by the
  Section~\ref{sec:baselines} RandomForest row reported
  here.} \\
\end{tabularx}
\end{table}
...
Plans~\cite{ndrc2022,ndrc2026,he2023,zhong2022}.
...
Bilgili et al.~\cite{bilgili2024} achieved 96.07\% accuracy
...
\subsection{Gate Value $g$: Geographic Mechanisms and
...
priority zones designated in the 14th and 15th Five-Year
Plans~\cite{ndrc2022,ndrc2026,he2023,zhong2022}. The
...
\bibitem{resch2023heliyon}
Resch E, Bohning C, West J, et al.
